...
describe("collectionTools", () => {
  ...
  describe("getRandomItem and takeRandomItem", () => {
    it("getRandomItem from Array", () => {
      const input = [1, 2, 3];
      const restore = mockRandom(0.4); // Math.floor(0.4 * 3) = 1
    ...
    it("getRandomItem from Set", () => {
      ...
      const restore = mockRandom(0.7); // Math.floor(0.7 * 3) = 2
    ...
    it("getRandomItem from Map (returns value)", () => {
      ...
      const restore = mockRandom(0.1); // Math.floor(0.1 * 2) = 0
    ...
    it("takeRandomItem from Set", () => {
      const input = new Set([1, 2, 3]);
      const restore = mockRandom(0.9); // index 2 -> item 3
    ...
    it("returns undefined for empty collections", () => {
      ...
      // Map/Set takeRandomItem handles empty
      expect(takeRandomItem(new Map())).toBeUndefined();
    });

    it("getRandomItem from Iterable (generic)", () => {
      const iterable = {
        *[Symbol.iterator]() {
          yield 10;
          yield 20;
      ...
      // Note: implementation of getRandomItem for Iterable falls through to throw error if not Array/Set/Map
      // Wait, let me check collectionTools.ts line 297-325
      expect(() => getRandomItem(iterable as any)).toThrow("unknown collection type object");
    });
  });
