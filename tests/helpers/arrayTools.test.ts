...
describe("arrayTools", () => {
  ...
  describe("random tools", () => {
    it("getRandom picks based on Math.random", () => {
      const arr = ["a", "b", "c"];
      const restore = mockRandom(0.1, 0.5, 0.9);
    ...
    it("getManyRandom picks multiple items", () => {
      const arr = ["a", "b", "c", "d"];
      const restore = mockRandom(0.1, 0.5); // 0.1*4=0.4->'a', then 0.5*3=1.5->'c' (if splice is used)

      // takeManyRandom implementation:
      // i = Math.floor(0.1 * 4) = 0. arr.splice(0,1) -> ['a'], arr is now ['b', 'c', 'd']
      // i = Math.floor(0.5 * 3) = 1. arr.splice(1,1) -> ['c'], arr is now ['b', 'd']
    ...
    it("takeRandom removes item from array", () => {
      const arr = ["a", "b", "c"];
      const restore = mockRandom(0.5); // 0.5 * 3 = 1.5 -> index 1 -> "b"
    ...
    it("shuffle shuffles array", () => {
      ...
      // i = 3, j = floor(random * 4)
      // i = 2, j = floor(random * 3)
      // i = 1, j = floor(random * 2)

      const restore = mockRandom(0.9, 0.1, 0.5);
