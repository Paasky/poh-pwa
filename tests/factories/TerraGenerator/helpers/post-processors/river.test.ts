...
describe('river', () => {
  ...
  it('marks other river downstream as major when merging', () => {
    ...
    for (const k of existing) {
      ...
      tiles[k] = { ...base, key: k, x, y, isFresh: true, riverKey: 'river:existing' }
    }

    // Register the other river object in the rivers map passed into river()
    const otherRiver = { key: 'river:existing', tileKeys: { value: existing.slice() } } as any
  ...
  it('stops when next to another river and promotes its neighbors and downstream to major', () => {
    ...
    const otherRiver = { key: otherKey, tileKeys: { value: otherTiles.slice() } } as any
