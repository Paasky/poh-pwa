...
const mkWorld = () => {
  const mk = (x: number, y: number) => ({ key: Tile.getKey(x, y), x, y, domain: { id: 'water' }, isSalt: false })
  const a = mk(0, 0)
  const b = mk(1, 0)
  const c = mk(2, 0)
  ...
  const gen = {
    getGameNeighbors: (coords: { x: number, y: number }) => N[Tile.getKey(coords.x, coords.y)] || [],
    getRegNeighbors: (coords: { x: number, y: number }) => N[Tile.getKey(coords.x, coords.y)] || [],
    getStratNeighbors: (coords: { x: number, y: number }) => N[Tile.getKey(coords.x, coords.y)] || [],
  }
