...
const heightMap: Record<string, number> = {
  center: 10,
  '1:n': 11, '1:nw': 12, '1:sw': 13, '1:s': 14, '1:se': 15, '1:ne': 16,
  '2:n': 21, '2:nw': 22, '2:sw': 23, '2:s': 24, '2:se': 25, '2:ne': 26, '2:e': 27, '2:w': 28,
}
...
const expK1PositionsFromCenterA = [
  // center
  0 + 0, heightMap.center, 0 - 0,
  // ring1 corners in order: n, nw, sw, s, se, ne
  0 + 0, heightMap['1:n'], 0 - 1,
  0 + (-Math.sqrt(3) / 2), heightMap['1:nw'], 0 - 0.5,
  0 + (-Math.sqrt(3) / 2), heightMap['1:sw'], 0 - (-0.5),
  0 + 0, heightMap['1:s'], 0 - (-1),
  0 + (Math.sqrt(3) / 2), heightMap['1:se'], 0 - (-0.5),
  0 + (Math.sqrt(3) / 2), heightMap['1:ne'], 0 - (0.5),
]
