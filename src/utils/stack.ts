...
function getAppFrames (opts?: { exclude?: string[]; limit?: number; err?: Error }): string[] {
  ...
  for (const line of lines) {
    ...
    for (const ex of exclude) {
      if (!ex) continue
      if (trimmed.includes(ex) || normalized.includes(ex)) { excluded = true; break }
    }
...
export function firstSrcFrame (opts?: { exclude?: string[] }): string {
  return getAppFrames({ exclude: opts?.exclude, limit: 1 })[0] ?? '(caller unknown)'
}
...
export function withCallerContext (message: string, extraExclude?: string[]): Error {
  ...
  const calledFromRaw = getAppFrames({ exclude: [...(extraExclude ?? []), thrownAt], err })[0] ?? '(caller unknown)'
  return new Error(`${message} — thrown at ${thrownAt} — called from ${formatCalledFrom(calledFromRaw)}`)
}
