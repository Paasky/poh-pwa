...
import Legends from '@/views/MapGenerator/components/Legends.vue'
  ...
  <div class="w-screen bg-gray-900 text-slate-100">
    <div class="w-full p-4 space-y-6">
      ...
      <div v-else>
        ...
        <Legends v-if="mapGenStore.toggles.showLegends"/>

        <!-- LEVEL TOGGLES -->
        <div class="mx-auto w-full max-w-full min-w-[72rem] mt-2 flex items-center gap-2">
          <UiButton :variant="mapGenStore.selectedLevel === 'strat' ? 'selected' : 'ghost'"
                    @click="mapGenStore.selectedLevel = 'strat'">
            Strategic
          </UiButton>
          <UiButton :variant="mapGenStore.selectedLevel === 'reg' ? 'selected' : 'ghost'"
                    @click="mapGenStore.selectedLevel = 'reg'">
            Region
          </UiButton>
          <UiButton :variant="mapGenStore.selectedLevel === 'game' ? 'selected' : 'ghost'"
