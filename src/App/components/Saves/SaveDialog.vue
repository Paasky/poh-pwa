...
// Utils & Stores
import { saveManager, type SaveMeta } from "@/Common/utils/saveManager";
  ...
  <v-dialog v-model="open" max-width="900" scrollable>
    <v-card rounded="lg" height="700">
      <v-card-title class="d-flex align-center">
        Save Games
        <v-spacer />
        <UiButton type="secondary" text="Upload Save" icon="upload" @click="fileInput?.click()" />
        <input
          ref="fileInput"
          type="file"
          accept=".json"
        ...
        <div v-if="hasDataBucket()" class="pa-4 bg-surface-variant">
          ...
          >
            <template #append-inner>
              <UiButton type="primary" text="Save" :disabled="!saveName.trim()" @click="doSave" />
            </template>
          </v-text-field>
        </div>
              ...
              <div class="flex-grow-1 d-flex flex-column ga-4 overflow-y-auto pa-4 h-100">
                <template v-if="selectedSave">
                  ...
                  <div class="d-flex ga-2">
                    ...
                    />
                    <v-spacer />
                    <UiButton type="primary" text="Load Game" @click="loadSave(selectedSave.id)" />
                  </div>
                </template>
