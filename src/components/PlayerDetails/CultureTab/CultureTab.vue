...
import { Culture } from '@/objects/gameObjects'
...
<template>
  <div>
    ...
    <div class="px-4 py-2 flex gap-8">
      <img :src="player.leader.image!" class="w-[20rem] h-[20rem] mb-2 mr-2 rounded-xl" :alt="player.leader.name"/>
      <div>
        <h3 class="text-xl select-none text-yellow-800 hover:text-yellow-600 cursor-pointer"
    ...
    <div v-if="culture.status === 'settled'" class="mb-4">
      ...
      />
      <div class="p-4">
        <UiButton v-if="culture.mustSelectTraits.positive + culture.mustSelectTraits.negative === 0"
        ...
        <UiCardGrid gap="gap-x-6 gap-y-4"
    ...
    <div>
      ...
      />
      <div class="p-4">
        <UiButton v-if="['canSettle', 'mustSettle'].includes(culture.status)" @click="manager.settle(culture)">
          Settle City
        </UiButton>
        ...
        <div v-if="culture.status !== 'settled'" class="text-sm grid gap-2 mb-4">
          <p v-if="culture.status === 'notSettled' || culture.status === 'canSettle'">
            Explore your surroundings to discover and learn from the nature around you.
            These skills will stay as your Cultural
          ...
          <UiCardGroup v-for="catData in objects.getClassTypesPerCategory('heritageType')">
            <div>
              ...
              >
                ...
                <UiButton @click="manager.addHeritagePoints(culture, catData.category.key, 10)">+10 p</UiButton>
