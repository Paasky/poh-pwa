...
export function cityTestData(): void {
  ...
  for (const [index, cityName] of ["Helsinki", "Paris"].entries()) {
    ...
    const city = new City(
      generateKey("city"),
      useCurrentContext().currentPlayer.key,
      Tile.getKey(x, y),
      cityName,
    ...
    bucket.set(city);
    useCurrentContext().currentPlayer.cityKeys.push(city.key);
    ...
    for (let i = 0; i < citizenCount; i++) {
      const cz = new Citizen(
        ...
        city.key,
        isHelsinki || i === 0
          ? useCurrentContext().currentPlayer.cultureKey
          : otherPlayer.cultureKey,
        useCurrentContext().currentPlayer.key,
        city.tileKey,
      );
      ...
      useCurrentContext().currentPlayer.citizenKeys.push(cz.key);
    ...
    if (isHelsinki) {
      const units = useCurrentContext().currentPlayer.units as Unit[];
    ...
    if (!isHelsinki) {
      const designs = useCurrentContext().currentPlayer.designs;
