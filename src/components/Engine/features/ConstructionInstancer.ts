/*
TODO: ConstructionInstancer (GPU instancing for constructions)

Purpose
- Render construction visuals (under construction/constructed) (buildings/wonders) using GPU instancing.

Planned Public API (no implementation yet)
- constructor(scene: SceneLike, world: WorldLike)
  -> keep internal Record/Map of tile GameKey -> construction GameKey
- build(): Promise<this> | this
  -> loop world tiles and add all existing tile.construction (use addMany for perf)
  -> keep internal Record/Map of tile GameKey -> construction GameKey
- add(d: InstanceDescriptor): this
  -> descriptor.gameKey should reference the Construction GameKey
