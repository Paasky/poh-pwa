...
export function getBaseKelp (scene: Scene): Mesh {
  const kelp = MeshBuilder.CreateCylinder(
    'kelpStalk',
    { height: 1.0, diameter: 0.07, tessellation: 3 },
    scene,
  )
  kelp.position.y = 1.0 / 2
  kelp.material = colorMat(scene, 0.0, 0.3, 0.0) // dark green
  kelp.isVisible = false
  return kelp
}

export function getBaseIce (scene: Scene): Mesh {
  const ice = MeshBuilder.CreateCylinder(
    'baseIce',
    { height: 0.3, diameterTop: 0, diameterBottom: 0.25, tessellation: 5 },
    scene,
  )
  ice.material = colorMat(scene, 0.95, 0.95, 1.0) // icy white
  ice.isVisible = false
