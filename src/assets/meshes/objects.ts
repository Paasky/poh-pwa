...
export const getBaseHuman = (scene: Scene): Mesh => {
  ...
  // Body (torso)
  const bodyH = 0.35
  const body = MeshBuilder.CreateCylinder('humanBody', { height: bodyH, diameter: 0.08, tessellation: 6 }, scene)
  ...
  const limbMaster = MeshBuilder.CreateCylinder('humanLimbMaster', { height: 0.28, diameter: 0.04, tessellation: 3 }, scene)
...
export const getBaseHorse = (scene: Scene): Mesh => {
  ...
  // Neck
  const neck = MeshBuilder.CreateCylinder('horseNeck', { height: 0.22, diameter: 0.08, tessellation: 6 }, scene)
  ...
  // Leg master
  const legMaster = MeshBuilder.CreateCylinder('horseLegMaster', { height: 0.32, diameter: 0.05, tessellation: 3 }, scene)
...
export const getBaseChariot = (scene: Scene): Mesh => {
  ...
  // Cart box
  const cart = MeshBuilder.CreateBox('chariotCart', { width: 0.32, height: 0.18, depth: 0.22 }, scene)
  cart.position = new Vector3(-0.15, 0.18 / 2 + 0.05, 0)
  cart.material = cartMat
  cart.parent = root

  // Wheels (thin cylinders) - instance
  const wheelMaster = MeshBuilder.CreateCylinder('chariotWheelMaster', { height: 0.04, diameter: 0.18, tessellation: 12 }, scene)
  ...
  const shaftMaster = MeshBuilder.CreateCylinder('chariotShaftMaster', { height: 0.4, diameter: 0.03, tessellation: 3 }, scene)
  ...
  // Optional yoke
  const yoke = MeshBuilder.CreateCylinder('chariotYoke', { height: 0.18, diameter: 0.03, tessellation: 3 }, scene)
...
export const getBaseRaft = (scene: Scene): Mesh => {
  ...
  const base = MeshBuilder.CreateCylinder('raftBase', { height: 0.06, diameter: 0.6, tessellation: 6 }, scene)
  ...
  // Mast
  const mast = MeshBuilder.CreateCylinder('raftMast', { height: 0.6, diameter: 0.035, tessellation: 3 }, scene)
...
export const getBaseGalley = (scene: Scene): Mesh => {
  ...
  const hull = MeshBuilder.CreateCylinder('galleyHull', { height: 0.06, diameter: 0.55, tessellation: 6 }, scene)
  ...
  // Oar master
  const oarMaster = MeshBuilder.CreateCylinder('galleyOarMaster', { height: 0.28, diameter: 0.025, tessellation: 3 }, scene)
  ...
  const mast = MeshBuilder.CreateCylinder('galleyMast', { height: 0.45, diameter: 0.03, tessellation: 3 }, scene)
