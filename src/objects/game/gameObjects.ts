...
export class City extends HasCitizens(
  HasPlayer(HasTile(HasUnits(GameObject))),
) {
  constructor(
    key: GameKey,
    playerKey: GameKey,
  ...
  private _citizenYields = computed((): Yields => {
    ...
    return new Yields(
      this.citizens.value.flatMap((c) => c.yields.value.only(inherit).all()),
    );
  });
...
export class Construction extends HasCitizens(
  CanHaveCity(HasTile(GameObject)),
) {
  constructor(
    key: GameKey,
    type: TypeObject,
...
export class Culture extends HasCitizens(HasPlayer(GameObject)) {
  ...
  region = computed(() =>
    objStore().getTypeObject(
      this.type.value.requires.filter(["regionType"]).allTypes[0] as TypeKey,
    ),
  );
...
export class Player extends HasCitizens(
  HasCulture(CanHaveReligion(HasUnits(GameObject))),
) {
  constructor(
    key: GameKey,
    name: string,
...
export class Religion extends HasCitizens(HasCity(HasPlayers(GameObject))) {
  ...
  static attrsConf: GameObjAttr[] = [
    { attrName: "name", attrNotRef: true },
    { attrName: "cityKey", related: { theirKeyAttr: "holyCityForKeys" } },
...
export class Tile extends HasCitizens(
  CanHaveCity(CanHavePlayer(HasUnits(GameObject))),
) {
  constructor(
    key: GameKey,
    x: number,
  ...
  river = computed(() =>
    this.riverKey ? (objStore().get(this.riverKey) as River) : null,
  );
...
export class TradeRoute extends HasPlayer(GameObject) {
  ...
  static attrsConf: GameObjAttr[] = [{ attrName: "playerKey" }];
}
