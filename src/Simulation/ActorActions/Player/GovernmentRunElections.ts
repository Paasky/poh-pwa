  ...
  handleAction(): IMutation<Government>[] {
    ...
    this.government.selectablePolicies.forEach((policy) => {
      ...
      countedPolicies[category][policy.key] = {
        ...
        citizens: filter(this.government.player.citizens, (citizen) => citizen.policy === policy)
          .size,
      };
    });
    ...
    mutation.payload.discontent = Math.max(
      0,
      roundToTenth(this.government.discontent - GovernmentConfig.elections.removeDiscontent),
    );

    // Set next elections in 25 turns
    mutation.payload.nextElection = GovernmentConfig.elections.everyNthTurn;
