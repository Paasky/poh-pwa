...
export class PlayerSelectType implements ISimAction {
  ...
  validateAction(): this {
    switch (this.type.class) {
      case "mythType":
        if (!this.player.religion?.selectableMyths.includes(this.type)) {
          throw new Error("This myth is not selectable by this player");
        }
        break;
      case "godType":
        if (!this.player.religion?.selectableGods.includes(this.type)) {
          throw new Error("This god is not selectable by this player");
        }
        break;
      case "dogmaType":
        if (!this.player.religion?.selectableDogmas.includes(this.type)) {
          throw new Error("This dogma is not selectable by this player");
        }
        break;
      case "heritageType":
        if (!this.player.culture.selectableHeritages.includes(this.type)) {
          throw new Error("This heritage is not selectable by this player");
        }
        break;
      case "traitType":
        if (!this.player.culture.selectableTraits.includes(this.type)) {
          throw new Error("This trait is not selectable by this player");
        }
        break;
      case "technologyType":
        if (this.player.research.researched.includes(this.type)) {
