...
export const createWorld = (size: WorldSize): WorldBundle => {
  ...
  function generateContinentsData (size: WorldSize, tilesByKey: Record<GameKey, Tile>): ContinentData[] {
    ...
    return selectedContinents.map((c, i): ContinentData => ({
      type: c,
      majorStarts,
      minorStarts
    }))
  ...
  function getRandomTile (size: WorldSize, tilesByKey: Record<GameKey, Tile>): Tile | undefined {
    // Random tile -4 to +4 x & y away (clamp to map size)
    const randX = Math.max(0, Math.min(size.x - 1, Math.floor(-4 + Math.random() * 8)))
    const randY = Math.max(0, Math.min(size.y - 1, Math.floor(-4 + Math.random() * 8)))
