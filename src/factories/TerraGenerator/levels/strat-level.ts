...
export class StratLevel {
    ...
    for (const y of ys) {
      for (const x of xs) {
        ...
        if (type === 'continentType') {
          ...
          while (
            this.gen.continents[continent.key].majorStarts.strat.length < this.gen.size.majorsPerContinent
        ...
        this.gen.stratTiles[key] = new GenTile(
          ...
          ocean,
          climate,
          ['caribbean', 'mediterranean'].includes(ocean.id) ? this.gen.seaTerrain : this.gen.oceanTerrain,
          this.gen.flat
        )
  ...
  generateEmpties (): StratLevel {
    ...
    while (emptyCoords.length > 0) {
      ...
      const landOnly = this.gen.size.seaLevel === 3 ? false : (
      ...
      const randNeighbors = landOnly ? neighbors.filter(n => n.domain === this.gen.land) : neighbors
      ...
      const preferredOcean = neighbors.find(n => n.terrain === this.gen.seaTerrain)
      ...
      this.gen.stratTiles[key].isSalt = preferredOcean.terrain === this.gen.seaTerrain || preferredOcean.terrain === this.gen.oceanTerrain
