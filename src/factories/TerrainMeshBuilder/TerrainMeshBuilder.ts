...
export class TerrainMeshBuilder {
  ...
  constructor (scene: Scene, size: Coords, tilesByKey: Record<string, Tile>, hexRingCount: number = 1) {
    this.scene = scene
    this.size = size
    ...
    for (let tileY = 0; tileY < this.size.y; tileY++) {
      for (let tileX = 0; tileX < this.size.x; tileX++) {
        ...
        buildHexGpuBuffer(
          ...
          this.points,
          this.triangles,
          (ringNumFromCenter, corner, edge) => this.getColor(
            center,
            tile,
            ringNumFromCenter,
            corner,
          ...
          (ringNumFromCenter, corner, edge) => this.getHeight(
            center,
            tile,
            ringNumFromCenter,
            corner,
  ...
  private getColor (center: HexMeshConf, tile: Tile, ringNumFromCenter: number, corner?: CompassHexCorner, edge?: CompassHexEdge) {
    // center vertex
  ...
  // If corner and edge are both undefined, assume it's the center vertex
  private getHeight (center: HexMeshConf, tile: Tile, ringNumFromCenter: number, corner?: CompassHexCorner, edge?: CompassHexEdge) {
    // center vertex
  ...
  private centerAffection<T> (ringNumFromCenter: number, fill: T): T[] {
    ...
    const centerEmphasis = ringNumFromCenter ? 1 : Math.round(2 * this.hexRingCount / ringNumFromCenter)
