...
function jitterWithinHex(points: EngineCoords[], magnitude: number): EngineCoords[] {
  ...
  for (const point of points) {
    const jitterX = rng.between(-magnitude, magnitude);
    const jitterZ = rng.between(-magnitude, magnitude);
