 ...
 */
export class Tetris {
  static randomOffsets(rng: Rng): Offset[] {
    const use3 = rng.chance(0.7);
    ...
    if (use3) {
      ...
      const base = rng.pick(shapes3Center)!;
      const rot = rng.intBetween(0, 3);
    ...
    const base = rng.pick(shapes2TopLeft)!;
