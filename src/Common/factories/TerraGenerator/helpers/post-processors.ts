...
import { Tetris } from "@/Common/factories/TerraGenerator/helpers/tetris";
import { generateKey } from "@/Common/Models/_GameModel";
import { Coords, getHexNeighborCoords, getTile } from "@/Common/Helpers/mapTools";
import { TerraGenerator } from "@/Common/factories/TerraGenerator/terra-generator";
import { AcceptResult, Snake } from "@/Common/factories/TerraGenerator/helpers/snake";
import { River } from "@/Common/Models/River";

export const removeOrphanArea = (tile: GenTile, neighbors: GenTile[], rng: Rng): void => {
...
export const makeIsland = (
  ...
  const offsets = Tetris.randomOffsets(gen.rng);
  const size = level === "reg" ? gen.regSize : gen.size;
  const tiles = level === "reg" ? gen.regTiles : gen.gameTiles;
  const elevType = gen.rng.chance(hillChance) ? gen.hill : gen.flat;
...
export const mountainRange = (
  ...
  return new Snake(
    ...
    (tile) => {
      ...
      tile.elevation =
        tile.elevation === mountain || tile.elevation === snowMountain || rng.chance(0.1)
          ? snowMountain
          : mountain;
