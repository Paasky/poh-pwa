...
import { ContinentData } from "@/Common/factories/TerraGenerator/config";
...
export class StratLevel {
    ...
    for (const y of ys) {
      for (const x of xs) {
        ...
        if (type === "continentType") {
          ...
          const continent = this.gen.rng.take(continents)!;
          ...
          while (
            ...
            const coords = this.gen.rng.take(
  ...
  generateEmpties(): StratLevel {
    ...
    while (emptyCoords.length > 0) {
      const randI = this.gen.rng.intBetween(0, emptyCoords.length - 1);
      ...
      const landOnly = this.gen.rng.chance(0.25);
      ...
      const rand = this.gen.rng.pick(randNeighbors)!;
    ...
    for (let y = 0; y < this.gen.stratSize.y; y++) {
      for (let x = 0; x < this.gen.stratSize.x; x++) {
        ...
        if (isLand) {
          ...
          if (this.gen.rng.chance(0.05)) tile.elevation = this.gen.hill;
          ...
          if (allAreSameDomain && this.gen.rng.chance(0.5)) {
