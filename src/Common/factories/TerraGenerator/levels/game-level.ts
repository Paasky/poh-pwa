...
export class GameLevel {
  ...
  fillFromReg(): GameLevel {
    for (let y = 0; y < this.gen.size.y; y++) {
      for (let x = 0; x < this.gen.size.x; x++) {
        ...
        const regTile = this.gen.rng.pick(regNeighbors)!;

        const elevChance = this.gen.rng.next();
        const elevation =
        ...
        if (!setFeatureLater) {
          ...
          tile.feature = this.gen.rng.chance(0.25)
            ? regTile.feature
  ...
  private postProcessPass1(): GameLevel {
    this.gen.forEachGameTile((tile) => {
      ...
      removeOrphanArea(tile, neighbors, this.gen.rng);
      removeOrphanTerrain(tile, neighbors, this.gen.rng);

      if (tile.elevation.id === "mountain") {
        // If it's a mountain, make 50% of flat neighbors into hills
        neighbors
          .filter((n) => n.elevation.id === "flat")
          .forEach((n) => (this.gen.rng.chance(0.5) ? (n.elevation = this.gen.hill) : null));
      }
      if (tile.elevation.id === "snowMountain") {
        ...
        neighbors
          ...
          .forEach((n) =>
            this.gen.rng.chance(0.5)
              ? (n.elevation = this.gen.hill)
              : (n.elevation = this.gen.mountain),
          );
      }
      ...
      if (tile.x % 3 === 1 && tile.y % 3 === 1) {
        ...
        // Add Starts
        if (regTile.isStart) {
          const startTile = this.gen.rng.pick(neighbors.filter((n) => n.canBeStart()))!;
  ...
  private addRivers(): GameLevel {
    ...
    for (const continent of Object.values(this.gen.continents)) {
      const candidates = this.gen.rng.shuffle([...candidatesPerContinent[continent.type.key]]);
      let riverCount = 0;
