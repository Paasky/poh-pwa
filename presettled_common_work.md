...
### 1. PohAction Enhancement

- [ ] Add `transactionId: string` to `PohAction`. Use `crypto.randomUUID()`.
- [ ] Add `timestamp: number` to `PohAction`.
- [ ] Ensure `ActionType` union in `PohAction.ts` is 1:1 with `public/data/types/locked/actionType.json`.

### 2. Model Performance & Logic

- [ ] **Visibility Getters**: Update `Player.visibleTileKeys` and `Unit.visibleTileKeys` to use the `this.computed()`
  decorator.
- [ ] **Culture Status**: Ensure `Culture` model supports the transition states: `unsettled` -> `canSettle` ->
  `mustSettle` -> `settled`.
...
- [ ] **Spatial Logic**: Ensure `getDistance` in `mapTools.ts` is used for both AI decision making (finding settle
  sites) and Simulation validation (4-hex distance rule).
