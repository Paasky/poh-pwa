...
# Usage:
#   bash poh.sh                      # default toggle: dev up if down, else down
#   bash poh.sh <number> [port]      # run a specific action non-interactively, optional port
#   bash poh.sh <word> [port]        # same actions with words, e.g. dev-up | dev-down | prod-up | prod-down | rebuild | status | fix | check | prep
...
run_data() {
  echo "Baking and Syncing Static Data (pnpm data:update)..."
  run_in_dev_container pnpm data:update
}
